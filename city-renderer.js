...
window.CityRenderer=(()=>{
  ...
  function create(gl,report=()=>{}){
    ...
    const vertex=`precision highp float;
      ...
      varying mediump vec3 vNormal; varying mediump vec2 vUV; varying mediump float vHeight; varying mediump float vSeed; varying mediump float vRoof;
      void main(){
        ...
        vec3 local=module*vec3(aMeta.z,height*smoothstep(.06,.72,uFocus),aMeta.w);
        ...
        vUV=vec2(mod(aFace.w,2.0),floor(aFace.w/2.0));vHeight=module.y*(3.0+aMeta.y*15.0);vSeed=aMeta.y;vRoof=aFace.y;
        gl_Position=uMatrix*vec4(p,1.0);
      }`;
    const fragment=`precision mediump float;
      uniform vec3 uSun; uniform float uDark; uniform mediump float uFocus; uniform int uMode;
      varying mediump vec3 vNormal; varying mediump vec2 vUV; varying mediump float vHeight; varying mediump float vSeed; varying mediump float vRoof;
      void main(){
        vec3 n=normalize(vNormal);
        vec3 ink=mix(vec3(.14,.17,.14),vec3(.71,.82,.75),uDark);
        if(uMode==1){gl_FragColor=vec4(ink,smoothstep(.04,.3,uFocus)*.72);return;}
        if(uMode==2){gl_FragColor=vec4(n*.38+.5,smoothstep(.04,.3,uFocus));return;}
        float light=.25+.75*max(0.0,dot(n,normalize(uSun)));
        vec3 paper=mix(vec3(.54,.57,.53),vec3(.97,.97,.94),light);
        vec3 night=mix(vec3(.055,.07,.065),vec3(.29,.34,.30),light);
        vec3 color=mix(paper,night,uDark);
        float border=min(min(vUV.x,1.0-vUV.x),min(vUV.y,1.0-vUV.y));
        float edge=1.0-smoothstep(.005,.025,border);
        float floorLine=(1.0-vRoof)*(1.0-smoothstep(.025,.065,min(fract(vHeight),1.0-fract(vHeight))));
        float windowBand=(1.0-vRoof)*step(.2,fract(vHeight))*step(fract(vHeight),.77);
        float column=step(.12,fract(vUV.x*3.0))*step(fract(vUV.x*3.0),.85);
        color=mix(color,ink,max(edge*.78,floorLine*.20));
        color=mix(color,ink,windowBand*column*.18);
        if(uDark>.5&&vSeed>.89)color=mix(color,vec3(.66,.82,.44),windowBand*column*.42);
        gl_FragColor=vec4(color,smoothstep(.04,.3,uFocus));
      }`;
    let program,shaders=[];
    try{
    ...
    const uniforms=Object.fromEntries(['uMatrix','uPose','uMix','uFocus','uSeparate','uSun','uDark','uMode'].map(name=>[name,gl.getUniformLocation(program,name)]));
  ...
  function fallback(context,{matrix,poses,from,to,mix,focus,route,separation,sun,dark,mode,width,height}){
    ...
    for(const {i} of candidates){
      ...
      const growth=Math.min(1,Math.max(0,(focus-.06)/.66));
      const local=p=>{
      ...
      for(const box of modules)city.faceCorners.forEach((corners,f)=>{
        ...
        faces.push({pts,depth:pts.reduce((s,p)=>s+p[2],0)/4,color:mode==='normals'?'rgb('+normal.map(n=>Math.round((n*.38+.5)*255)).join(',')+')':`rgb(${value},${value+2},${value})`});
      });
    }
    context.globalAlpha=Math.min(1,focus*3);context.lineWidth=.65;context.strokeStyle=dark?'#abc4b2':'#343b33';
    faces.sort((a,b)=>b.depth-a.depth).forEach(({pts,color})=>{
